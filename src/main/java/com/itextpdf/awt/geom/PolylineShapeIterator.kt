...
internal constructor(
        ...
        protected var poly: PolylineShape,
        /** an affine transformation to be performed on the polyline.  */
        protected var affine: AffineTransform?) : PathIterator {
    ...
    override fun currentSegment(coords: DoubleArray): Int {
        if (isDone) {
            throw NoSuchElementException(MessageLocalization.getComposedMessage("line.iterator.out.of.bounds"))
        }
        val type = if (index == 0) PathIterator.SEG_MOVETO else PathIterator.SEG_LINETO
        coords[0] = poly.x[index].toDouble()
        coords[1] = poly.y[index].toDouble()
        if (affine != null) {
    ...
    override fun currentSegment(coords: FloatArray): Int {
        ...
        val type = if (index == 0) PathIterator.SEG_MOVETO else PathIterator.SEG_LINETO
        coords[0] = poly.x[index].toFloat()
        coords[1] = poly.y[index].toFloat()
        if (affine != null) {
            affine!!.transform(coords, 0, coords, 0, 1)
        }
        return type
