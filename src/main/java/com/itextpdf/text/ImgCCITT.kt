...
class ImgCCITT : Image {
     ...
     * *
     * @param typeCCITT the type of compression in `data`. It can be
    ...
    constructor(width: Int, height: Int, reverseBits: Boolean, typeCCITT: Int, parameters: Int, data: ByteArray) : super(null as URL) {
        if (typeCCITT != Element.CCITTG4 && typeCCITT != Element.CCITTG3_1D && typeCCITT != Element.CCITTG3_2D)
            throw BadElementException(MessageLocalization.getComposedMessage("the.ccitt.compression.type.must.be.ccittg4.ccittg3.1d.or.ccittg3.2d"))
        if (reverseBits)
