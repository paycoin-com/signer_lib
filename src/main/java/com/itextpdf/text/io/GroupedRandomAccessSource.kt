 ...
 * @since 5.3.5
 */
internal open class GroupedRandomAccessSource
/**
...
constructor(sources: Array<RandomAccessSource>) : RandomAccessSource {
    ...
    init {
        this.sources = arrayOfNulls<SourceEntry>(sources.size)

        var totalSize: Long = 0
        for (i in sources.indices) {
            this.sources[i] = SourceEntry(i, sources[i], totalSize)
            totalSize += sources[i].length()
        }
        size = totalSize
        currentSourceEntry = this.sources[sources.size - 1]
     ...
     * Subclasses should override this method if they are able to compute the source index more efficiently (for example [FileChannelRandomAccessSource] takes advantage of fixed size page buffers to compute the index)
     * @param offset the offset
     * *
     * @return the index of the input source that contains the specified offset, or 0 if unknown
     */
    protected open fun getStartingSourceIndex(offset: Long): Int {
     ...
     */
    @Throws(IOException::class)
    private fun getSourceEntryForOffset(offset: Long): SourceEntry? {
        if (offset >= size)
            return null
        ...
        val startAt = getStartingSourceIndex(offset)

        for (i in startAt..sources.size - 1) {
     ...
     */
    @Throws(IOException::class)
    protected open fun sourceReleased(source: RandomAccessSource) {
     ...
     */
    @Throws(IOException::class)
    protected open fun sourceInUse(source: RandomAccessSource) {
        // by default, do nothing
    }
    ...
    override fun get(position: Long): Int {
        val entry = getSourceEntryForOffset(position) ?: return -1 // we have run out of data to read from
    ...
    override fun get(position: Long, bytes: ByteArray, off: Int, len: Int): Int {
        ...
        var off = off
        var entry: SourceEntry? = getSourceEntryForOffset(position) ?: return -1 // we have run out of data to read from
        ...
        while (remaining > 0) {
            ...
            val count = entry.source.get(offN, bytes, off, remaining)
            if (count == -1)
                break
        ...
        return if (remaining == len) -1 else len - remaining
    }
    ...
    (
         ...
         */
        fun offsetN(absoluteOffset: Long): Long {
