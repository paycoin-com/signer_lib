...
(
         ...
         * The source
         */
        private val source: RandomAccessSource) : RandomAccessSource {

    private val getBuffer: ByteArray
    private var getBufferStart: Long = -1
    private var getBufferEnd: Long = -1

    init {

        this.getBuffer = ByteArray(Math.min(Math.max(source.length() / 4, 1), 4096.toLong()).toInt())
    ...
    override fun get(position: Long): Int {
        if (position < getBufferStart || position > getBufferEnd) {
            val count = source.get(position, getBuffer, 0, getBuffer.size)
            if (count == -1)
                return -1
            getBufferStart = position
            getBufferEnd = position + count - 1
        }
        val bufPos = (position - getBufferStart).toInt()
        return 0xff and getBuffer[bufPos]
    }
