...
class TempFileCache @Throws(IOException::class)
constructor(private val filename: String) {

    inner class ObjectPosition internal constructor(internal var offset:

                                                    Long, internal var length: Int)
    ...
    init {
        ...
        cache = RandomAccessFile(filename, "rw")

        baos = ByteArrayOutputStream()
    }

    @Throws(IOException::class)
    fun put(obj: PdfObject): ObjectPosition {
        baos.reset()
        ...
        return ObjectPosition(offset, size.toInt())
    }

    @Throws(IOException::class, ClassNotFoundException::class)
    operator fun get(pos: ObjectPosition?): PdfObject {
        var obj: PdfObject? = null
        if (pos != null) {
            cache!!.seek(pos.offset)
            cache!!.read(getBuffer(pos.length), 0, pos.length)

            val ois = ObjectInputStream(ByteArrayInputStream(getBuffer(pos.length)))
            try {
                obj = ois.readObject() as PdfObject
            } finally {
                ois.close()
            }
        }
