...
class Version {
     ...
     * in every PDF that is created or manipulated using iText.
     */
    private var iTextVersion = "$product $release \u00a92000-2015 iText Group NV"
    /**
     * The license key.
     */
    /**
    ...
    companion object {
            ...
            get() {
                if (version == null) {
                    ...
                    synchronized (version) {
                        try {
                            val klass = Class.forName("com.itextpdf.license.LicenseKey")
                            val m = klass.getMethod("getLicenseeInfo")
                            val info = m.invoke(klass.newInstance()) as Array<String>
                            if (info[3] != null && info[3].trim { it <= ' ' }.length > 0) {
                                version!!.key = info[3]
                            } else {
                            ...
                            if (info[4] != null && info[4].trim { it <= ' ' }.length > 0) {
                                version!!.iTextVersion = info[4]
                            } else if (info[2] != null && info[2].trim { it <= ' ' }.length > 0) {
                            ...
                            } else if (info[0] != null && info[0].trim { it <= ' ' }.length > 0) {
         ...
         */
        val isAGPLVersion: Boolean
            get() = instance.getVersion().indexOf(AGPL) > 0
    }
}
