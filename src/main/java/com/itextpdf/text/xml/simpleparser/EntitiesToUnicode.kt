...
object EntitiesToUnicode {
     ...
     */
    fun decodeEntity(name: String): Char {
        if (name.startsWith("#x")) {
            try {
                return Integer.parseInt(name.substring(2), 16).toChar()
        ...
        if (name.startsWith("#")) {
            try {
                return Integer.parseInt(name.substring(1)).toChar()
            } catch (nfe: NumberFormatException) {
                return '\0'
            }
    ...
    fun decodeString(s: String): String {
        var pos_amp = s.indexOf('&')
        if (pos_amp == -1) return s

        var pos_sc: Int
        var pos_a: Int
        val buf = StringBuffer(s.substring(0, pos_amp))
        var replace: Char
        while (true) {
