...
class BarcodePDF417 {
    ...
    private var macroSegmentCount = 0
    private var macroSegmentId = -1
    private var macroFileId: String? = null
    private var macroIndex: Int = 0
    protected var bitPtr: Int = 0
    protected var cwPtr: Int = 0
    protected var segmentList: SegmentList? = null
     ...
     * *
     * @see .setMacroSegmentCount
     */
    fun setMacroSegmentId(id: Int) {
     ...
     * *
     * @see .setMacroSegmentId
     */
    fun setMacroSegmentCount(cnt: Int) {
     ...
     */
    fun setMacroFileId(id: String) {
        this.macroFileId = id
    }

    protected fun checkSegmentType(segment: Segment?, type: Char): Boolean {
        if (segment == null)
            return false
        return segment.type == type
    }

    protected fun getSegmentLength(segment: Segment?): Int {
        if (segment == null)
            return 0
    ...
    protected fun outCodeword17(codeword: Int) {
        var codeword = codeword
        var bytePtr = bitPtr / 8
        val bit = bitPtr - bytePtr * 8
        outBits[bytePtr++] = outBits[bytePtr++] or (codeword shr 9 + bit).toByte()
        outBits[bytePtr++] = outBits[bytePtr++] or (codeword shr 1 + bit).toByte()
        codeword = codeword shl 8
        outBits[bytePtr] = outBits[bytePtr] or (codeword shr 1 + bit).toByte()
        bitPtr += 17
    }

    protected fun outCodeword18(codeword: Int) {
        var codeword = codeword
        var bytePtr = bitPtr / 8
        val bit = bitPtr - bytePtr * 8
        outBits[bytePtr++] = outBits[bytePtr++] or (codeword shr 10 + bit).toByte()
        outBits[bytePtr++] = outBits[bytePtr++] or (codeword shr 2 + bit).toByte()
        codeword = codeword shl 8
        outBits[bytePtr] = outBits[bytePtr] or (codeword shr 2 + bit).toByte()
        if (bit == 7)
            outBits[++bytePtr] = outBits[++bytePtr] or 0x80
        bitPtr += 18
    }

    protected fun outCodeword(codeword: Int) {
        outCodeword17(codeword)
    }

    protected fun outStopPattern() {
        outCodeword18(STOP_PATTERN)
    }

    protected fun outStartPattern() {
        outCodeword17(START_PATTERN)
    }

    protected fun outPaintCode() {
        var codePtr = 0
        bitColumns = START_CODE_SIZE * (codeColumns + 3) + STOP_SIZE
        val lenBits = ((bitColumns - 1) / 8 + 1) * codeRows
        outBits = ByteArray(lenBits)
        for (row in 0..codeRows - 1) {
            bitPtr = ((bitColumns - 1) / 8 + 1) * 8 * row
            val rowMod = row % 3
            val cluster = CLUSTERS[rowMod]
            outStartPattern()
            var edge = 0
            when (rowMod) {
                0 -> edge = 30 * (row / 3) + (codeRows - 1) / 3
                1 -> edge = 30 * (row / 3) + errorLevel * 3 + (codeRows - 1) % 3
                else -> edge = 30 * (row / 3) + codeColumns - 1
            }
            outCodeword(cluster[edge])

            for (column in 0..codeColumns - 1) {
                outCodeword(cluster[codewords[codePtr++]])
            }

            when (rowMod) {
                0 -> edge = 30 * (row / 3) + codeColumns - 1
                1 -> edge = 30 * (row / 3) + (codeRows - 1) / 3
                else -> edge = 30 * (row / 3) + errorLevel * 3 + (codeRows - 1) % 3
            }
            outCodeword(cluster[edge])
            outStopPattern()
        }
        if (options and PDF417_INVERT_BITMAP != 0) {
            for (k in outBits!!.indices)
                outBits[k] = outBits[k] xor 0xff
        }
    }

    protected fun calculateErrorCorrection(dest: Int) {
        if (errorLevel < 0 || errorLevel > 8)
            errorLevel = 0
        val A = ERROR_LEVEL[errorLevel]
        val Alength = 2 shl errorLevel
        for (k in 0..Alength - 1)
            codewords[dest + k] = 0
        val lastE = Alength - 1
        for (k in 0..lenCodewords - 1) {
            val t1 = codewords[k] + codewords[dest]
            for (e in 0..lastE) {
                val t2 = t1 * A[lastE - e] % MOD
                val t3 = MOD - t2
                codewords[dest + e] = ((if (e == lastE) 0 else codewords[dest + e + 1]) + t3) % MOD
            }
        }
        for (k in 0..Alength - 1)
    ...
    protected fun getTextTypeAndValue(maxLength: Int, idx: Int): Int {
        return getTextTypeAndValue(text, maxLength, idx)
    }

    private fun textCompaction(input: ByteArray, start: Int, length: Int) {
        var length = length
        val dest = IntArray(ABSOLUTE_MAX_TEXT_SIZE * 2)
        var mode = ALPHA
        var ptr = 0
        var fullBytes = 0
        var v = 0
        ...
        while (k < length) {
            ...
            if (v and ISBYTE != 0) {
                if (ptr and 1 != 0) {
                    //add a padding word
                    dest[ptr++] = PAL
                    mode = if (mode and PUNCTUATION != 0) ALPHA else mode
                }
                dest[ptr++] = BYTESHIFT
                dest[ptr++] = v and 0xff
                fullBytes += 2
            ...
            when (mode) {
                ALPHA -> if (v and LOWER != 0) {
                    dest[ptr++] = LL
                    dest[ptr++] = v and 0xff
                    mode = LOWER
                } else if (v and MIXED != 0) {
                    dest[ptr++] = ML
                    dest[ptr++] = v and 0xff
                    mode = MIXED
                } else if (getTextTypeAndValue(input, length, k + 1) and getTextTypeAndValue(input, length, k + 2) and PUNCTUATION != 0) {
                    dest[ptr++] = ML
                    dest[ptr++] = PL
                    dest[ptr++] = v and 0xff
                    mode = PUNCTUATION
                ...
                LOWER -> if (v and ALPHA != 0) {
                    if (getTextTypeAndValue(input, length, k + 1) and getTextTypeAndValue(input, length, k + 2) and ALPHA != 0) {
                        dest[ptr++] = ML
                        dest[ptr++] = AL
                        mode = ALPHA
                    } else {
                ...
                MIXED -> if (v and LOWER != 0) {
                    dest[ptr++] = LL
                    dest[ptr++] = v and 0xff
                    mode = LOWER
                } else if (v and ALPHA != 0) {
                    dest[ptr++] = AL
                    dest[ptr++] = v and 0xff
                    mode = ALPHA
                } else if (getTextTypeAndValue(input, length, k + 1) and getTextTypeAndValue(input, length, k + 2) and PUNCTUATION != 0) {
        ...
        if (size + cwPtr > MAX_DATA_CODEWORDS) {
            throw IndexOutOfBoundsException(MessageLocalization.getComposedMessage("the.text.is.too.big"))
        }
        length = ptr
        ptr = 0
        while (ptr < length) {
    ...
    protected fun textCompaction(start: Int, length: Int) {
        textCompaction(text, start, length)
    }

    protected fun basicNumberCompaction(start: Int, length: Int) {
        basicNumberCompaction(text, start, length)
    }

    private fun basicNumberCompaction(input: ByteArray, start: Int, length: Int) {
        var length = length
        val ret = cwPtr
        val retLast = length / 3
        var ni: Int
        var k: Int
        cwPtr += retLast + 1
        k = 0
    ...
    private fun numberCompaction(input: ByteArray, start: Int, length: Int) {
        var length = length
        val full = length / 44 * 15
        var size = length % 44
        var k: Int
        if (size == 0)
            size = full
        ...
        k = start
        while (k < length) {
            size = if (length - k < 44) length - k else 44
            basicNumberCompaction(input, k, size)
            k += 44
        }
    }

    protected fun numberCompaction(start: Int, length: Int) {
        numberCompaction(text, start, length)
    }

    protected fun byteCompaction6(start: Int) {
        var length = 6
        val ret = cwPtr
        val retLast = 4
        var ni: Int
        ...
        while (ni < length) {
            ...
            // add the digit
            codewords[ret + retLast] += text!![ni] and 0xff
            // propagate carry
            k = retLast
            while (k > 0) {
                codewords[ret + k - 1] += codewords[ret + k] / 900
    ...
    internal fun byteCompaction(start: Int, length: Int) {
        var length = length
        var k: Int
        var j: Int
        ...
        k = start
        while (k < length) {
            size = if (length - k < 44) length - k else 6
            if (size < 6) {
                j = 0
                while (j < size) {
                    codewords[cwPtr++] = text!![k + j] and 0xff
                    ++j
                }
            } else {
                byteCompaction6(k)
    ...
    internal fun breakString() {
        ...
        if (options and PDF417_FORCE_BINARY != 0) {
            segmentList!!.add('B', 0, textLength)
            return
        }
        k = 0
        while (k < textLength) {
            c = (text!![k] and 0xff).toChar()
            if (c >= '0' && c <= '9') {
                if (nd == 0)
                    startN = k
                ++nd
            ...
            if (nd >= 13) {
                if (lastP != startN) {
                    c = (text!![lastP] and 0xff).toChar()
                    lastTxt = c >= ' ' && c.toInt() < 127 || c == '\r' || c == '\n' || c == '\t'
                    j = lastP
                    while (j < startN) {
                        c = (text!![j] and 0xff).toChar()
                        txt = c >= ' ' && c.toInt() < 127 || c == '\r' || c == '\n' || c == '\t'
                        if (txt != lastTxt) {
                            segmentList!!.add(if (lastTxt) 'T' else 'B', lastP, j)
                            lastP = j
                            lastTxt = txt
                        }
                        ++j
                    }
                    segmentList!!.add(if (lastTxt) 'T' else 'B', lastP, startN)
                }
        ...
        if (segmentList!!.size() == 1 && (v = segmentList!![0]).type == 'T' && getSegmentLength(v) >= 8) {
            k = v.start
            while (k < v.end) {
                c = (text!![k] and 0xff).toChar()
                if (c < '0' || c > '9')
    ...
    protected fun assemble() {
        ...
        while (k < segmentList!!.size()) {
            ...
            when (v.type) {
                ...
                'B' -> {
                    codewords[cwPtr++] = if (getSegmentLength(v) % 6 != 0) BYTE_MODE else BYTE_MODE_6
        ...
        if (options and PDF417_USE_MACRO != 0) {
            macroCodes()
        }

    }

    private fun macroCodes() {
        if (macroSegmentId < 0) {
            throw IllegalStateException(MessageLocalization.getComposedMessage("macrosegmentid.must.be.gt.eq.0"))
        }
        if (macroSegmentId >= macroSegmentCount) {
            throw IllegalStateException(MessageLocalization.getComposedMessage("macrosegmentid.must.be.lt.macrosemgentcount"))
        }
        if (macroSegmentCount < 1) {
            throw IllegalStateException(MessageLocalization.getComposedMessage("macrosemgentcount.must.be.gt.0"))
        }
    ...
    private fun append(`in`: Int, len: Int) {
        val sb = StringBuffer(len + 1)
        sb.append(Integer.toString(`in`))
        for (i in sb.length..len - 1) {
            sb.insert(0, "0")
        }

        val bytes = PdfEncodings.convertToBytes(sb.toString(), "cp437")
        numberCompaction(bytes, 0, bytes.size)
    }

    private fun append(s: String) {
        val bytes = PdfEncodings.convertToBytes(s, "cp437")
        textCompaction(bytes, 0, bytes.size)
    }

    protected fun dumpList() {
        if (segmentList!!.size() == 0)
            return
        for (k in 0..segmentList!!.size() - 1) {
            val v = segmentList!![k]
            val len = getSegmentLength(v)
            val c = CharArray(len)
            for (j in 0..len - 1) {
                c[j] = (text!![v.start + j] and 0xff).toChar()
                if (c[j] == '\r')
                    c[j] = '\n'
            }
            val sb = StringBuffer()
    ...
    fun paintCode() {
        ...
        var tot: Int
        var pad: Int
        if (options and PDF417_USE_RAW_CODEWORDS != 0) {
            if (lenCodewords > MAX_DATA_CODEWORDS || lenCodewords < 1 || lenCodewords != codewords[0]) {
                throw IllegalArgumentException(MessageLocalization.getComposedMessage("invalid.codeword.size"))
            }
        } else {
            if (text == null)
                throw NullPointerException(MessageLocalization.getComposedMessage("text.cannot.be.null"))
            if (text!!.size > ABSOLUTE_MAX_TEXT_SIZE) {
                throw IndexOutOfBoundsException(MessageLocalization.getComposedMessage("the.text.is.too.big"))
            }
        ...
        if (options and PDF417_USE_ERROR_LEVEL == 0) {
            if (lenCodewords < 41)
                errorLevel = 2
        ...
        var fixedColumn = options and PDF417_FIXED_ROWS == 0
        var skipRowColAdjust = false
        tot = lenCodewords + lenErr
        if (options and PDF417_FIXED_RECTANGLE != 0) {
        ...
        } else if (options and (PDF417_FIXED_COLUMNS or PDF417_FIXED_ROWS) == 0) {
            val c: Double
            val b: Double
            fixedColumn = true
            if (aspectRatio < 0.001)
                ...
                aspectRatio = 1000f
            b = (73 * aspectRatio - 4).toDouble()
            c = (-b + Math.sqrt(b * b + 4f * 17f * aspectRatio * (lenCodewords + lenErr).toFloat() * yHeight)) / (2f * 17f * aspectRatio)
            codeColumns = (c + 0.5).toInt()
            if (codeColumns < 1)
                codeColumns = 1
            else if (codeColumns > 30)
                codeColumns = 30
    ...
    val image: Image
        ...
        get() {
            paintCode()
            val g4 = CCITTG4Encoder.compress(outBits, bitColumns, codeRows)
            return Image.getInstance(bitColumns, codeRows, false, Image.CCITTG4, if (options and PDF417_INVERT_BITMAP == 0) 0 else Image.CCITT_BLACKIS1, g4, null)
        }
     ...
     */
    val codewords = IntArray(MAX_DATA_CODEWORDS + 2)
    ...
    protected class Segment(var type: Char, var start: Int, var end: Int)

    protected class SegmentList {
        protected var list = ArrayList<Segment>()

        fun add(type: Char, start: Int, end: Int) {
            list.add(Segment(type, start, end))
        }

        operator fun get(idx: Int): Segment? {
    ...
    fun placeBarcode(cb: PdfContentByte, foreground: BaseColor, moduleHeight: Float, moduleWidth: Float) {
        paintCode()
        val stride = (bitColumns + 7) / 8
        cb.setColorFill(foreground)
        for (k in 0..codeRows - 1) {
            val p = k * stride
            for (j in 0..bitColumns - 1) {
                var b = outBits!![p + j / 8] and 0xff
                b = b shl j % 8
                if (b and 0x80 != 0) {
    ...
    val barcodeSize: Rectangle
        get() {
            paintCode()
            return Rectangle(0f, 0f, bitColumns.toFloat(), codeRows.toFloat())
        }
    ...
    fun createAwtImage(foreground: java.awt.Color, background: java.awt.Color): java.awt.Image {
        ...
        paintCode()
        val h = yHeight.toInt()
        val pix = IntArray(bitColumns * codeRows * h)
        val stride = (bitColumns + 7) / 8
        var ptr = 0
        for (k in 0..codeRows - 1) {
            val p = k * stride
        ...
        val img = canvas.createImage(java.awt.image.MemoryImageSource(bitColumns, codeRows * h, pix, 0, bitColumns))
        return img
    }
    ...
    companion object {
        ...
        protected fun maxPossibleErrorLevel(remain: Int): Int {
            var level = 8
            var size = 512
            while (level > 0) {
                if (remain >= size)
        ...
        protected val START_PATTERN = 0x1fea8
        protected val STOP_PATTERN = 0x3fa29
        protected val START_CODE_SIZE = 17
        protected val STOP_SIZE = 18
        protected val MOD = 929
        protected val ALPHA = 0x10000
        protected val LOWER = 0x20000
        protected val MIXED = 0x40000
        protected val PUNCTUATION = 0x80000
        protected val ISBYTE = 0x100000
        protected val BYTESHIFT = 913
        protected val PL = 25
        protected val LL = 27
        protected val AS = 27
        protected val ML = 28
