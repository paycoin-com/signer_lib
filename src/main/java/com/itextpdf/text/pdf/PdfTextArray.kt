 ...
 *
 * A TextArray is used with the operator TJ in PdfText.
...
class PdfTextArray {
    internal var arrayList = ArrayList<Any>()
    ...
    private var lastNum: Float? = null
     ...
     */
    fun add(number: PdfNumber) {
        add(number.doubleValue().toFloat())
    }

    fun add(number: Float) {
        if (number != 0f) {
            if (lastNum != null) {
                lastNum = number + lastNum!!.toFloat()
                if (lastNum!!.toFloat() != 0f) {
                    replaceLast(lastNum)
                } else {
