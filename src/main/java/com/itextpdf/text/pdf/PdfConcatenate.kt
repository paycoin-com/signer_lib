...
@JvmOverloads constructor(os: OutputStream, smart: Boolean = false) {
    ...
    init {
        document = Document()
        if (smart)
            writer = PdfSmartCopy(document, os)
        else
            writer = PdfCopy(document, os)
    }
    ...
    fun addPages(reader: PdfReader): Int {
        open()
