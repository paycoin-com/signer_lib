 ...
 */
class ArrayBasedStringTokenizer(tokens: Array<String>) {
    ...
    fun tokenize(text: String): Array<String> {
        ...
        var endIndexOfpreviousMatch = 0

        while (matcher.find()) {

            val startIndexOfMatch = matcher.start()

            val previousToken = text.substring(endIndexOfpreviousMatch, startIndexOfMatch)

            if (previousToken.length > 0) {
                tokens.add(previousToken)
            }

            val currentMatch = matcher.group()
        ...
        val tail = text.substring(endIndexOfpreviousMatch, text.length)

        if (tail.length > 0) {
            tokens.add(tail)
        }

        return tokens.toArray<String>(arrayOfNulls<String>(0))

    }

    private fun getRegexFromTokens(tokens: Array<String>): String {
        val regexBuilder = StringBuilder(100)
