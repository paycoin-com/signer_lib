 ...
 */
@JvmOverloads constructor(d: Calendar = GregorianCalendar()) : PdfString() {
    init {
        val date = StringBuffer("D:")
        ...
        var timezone = (d.get(Calendar.ZONE_OFFSET) + d.get(Calendar.DST_OFFSET)) / (60 * 60 * 1000)
        if (timezone == 0) {
            date.append('Z')
        } else if (timezone < 0) {
            date.append('-')
        ...
        if (timezone != 0) {
            ...
            val zone = Math.abs((d.get(Calendar.ZONE_OFFSET) + d.get(Calendar.DST_OFFSET)) / (60 * 1000)) - timezone * 60
    ...
    private fun setLength(i: Int, length: Int): String {
     ...
     */
    val w3CDate: String
        get() = getW3CDate(value)

    companion object {

        private val DATE_SPACE = intArrayOf(Calendar.YEAR, 4, 0, Calendar.MONTH, 2, -1, Calendar.DAY_OF_MONTH, 2, 0, Calendar.HOUR_OF_DAY, 2, 0, Calendar.MINUTE, 2, 0, Calendar.SECOND, 2, 0)
         ...
         * @return a formatted date
         */
        fun getW3CDate(d: String): String {
            var d = d
            if (d.startsWith("D:"))
                d = d.substring(2)
            val sb = StringBuffer()
         ...
         * *
         * @return a Calendar representing the date or null if the string
         * * was not a date
         */
        fun decode(s: String): Calendar? {
            var s = s
            try {
                if (s.startsWith("D:"))
                    s = s.substring(2)
                ...
                if (idx >= 0) {
                    slen = idx
                    calendar = GregorianCalendar(SimpleTimeZone(0, "ZPDF"))
                } else {
                    var sign = 1
                    idx = s.indexOf('+')
                    if (idx < 0) {
                    ...
                    if (idx < 0)
                        ...
                        var offset = Integer.parseInt(s.substring(idx + 1, idx + 3)) * 60
                        if (idx + 5 < s.length)
                            offset += Integer.parseInt(s.substring(idx + 4, idx + 6))
                        calendar = GregorianCalendar(SimpleTimeZone(offset * sign * 60000, "ZPDF"))
                        slen = idx
                    }
                }
                calendar.clear()
