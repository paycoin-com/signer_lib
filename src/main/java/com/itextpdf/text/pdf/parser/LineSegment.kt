...
(
     ...
     */
    val length: Float
        get() = endPoint.subtract(startPoint).length()
     ...
     * @since 5.0.2
     */
    val boundingRectange: Rectangle2D.Float
        get() {
            val x1 = startPoint.get(Vector.I1)
            val y1 = startPoint.get(Vector.I2)
            val x2 = endPoint.get(Vector.I1)
            val y2 = endPoint.get(Vector.I2)
            return Rectangle2D.Float(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2 - y1))
        }
     ...
     */
    fun transformBy(m: Matrix): LineSegment {
        val newStart = startPoint.cross(m)
        val newEnd = endPoint.cross(m)
