 ...
 */
(controlPoints: List<Point2D>) : Shape {
    ...
    init {
        this.basePoints = ArrayList(controlPoints)
    }

    /**
     * You can adjust precision of the approximation by varying the following
     * parameters: [.curveCollinearityEpsilon], [.distanceToleranceSquare],
    ...
    val piecewiseLinearApproximation: List<Point2D>
        get() {
            ...
            points.add(basePoints[0])

            recursiveApproximation(basePoints[0].x.toDouble(), basePoints[0].y.toDouble(),
                    basePoints[1].x.toDouble(), basePoints[1].y.toDouble(),
                    basePoints[2].x.toDouble(), basePoints[2].y.toDouble(),
                    basePoints[3].x.toDouble(), basePoints[3].y.toDouble(), points)

            points.add(basePoints[basePoints.size - 1])
            return points
        }

    // Based on the De Casteljau's algorithm
    private fun recursiveApproximation(x1: Double, y1: Double, x2: Double, y2: Double,
                                       x3: Double, y3: Double, x4: Double, y4: Double, points: MutableList<Point2D>) {
        ...
        val d2 = Math.abs((x2 - x4) * dy - (y2 - y4) * dx)

        // |Ax3 + Bx3 + C|
        val d3 = Math.abs((x3 - x4) * dy - (y3 - y4) * dx)
        ...
        if (d2 > curveCollinearityEpsilon || d3 > curveCollinearityEpsilon) {
            ...
            if ((d2 + d3) * (d2 + d3) <= distanceToleranceSquare * (dx * dx + dy * dy)) {
                points.add(Point2D.Double(x1234, y1234))
                return
            }
    ...
    companion object {
         ...
         */
        var curveCollinearityEpsilon = 1.0e-30
         ...
         */
        var distanceToleranceSquare = 0.025
         ...
         */
        var distanceToleranceManhattan = 0.4
    }
}
