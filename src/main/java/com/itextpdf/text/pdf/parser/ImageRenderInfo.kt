...
class ImageRenderInfo {
    ...
    private val inlineImageInfo: InlineImageInfo?
    /** the color space associated with the image  */
    private val colorSpaceDictionary: PdfDictionary
    /** the image object to be rendered, if it has been parsed already.  Null otherwise.  */
    private var imageObject: PdfImageObject? = null

    private constructor(gs: GraphicsState, ref: PdfIndirectReference, colorSpaceDictionary: PdfDictionary) {
        this.gs = gs
        this.ref = ref
    ...
    private constructor(gs: GraphicsState, inlineImageInfo: InlineImageInfo, colorSpaceDictionary: PdfDictionary) {
        this.gs = gs
        this.ref = null
    ...
    @Throws(IOException::class)
    private fun prepareImageObject() {
        if (imageObject != null)
            return

        if (ref != null) {
            val stream = PdfReader.getPdfObject(ref) as PRStream
            imageObject = PdfImageObject(stream, colorSpaceDictionary)
        } else if (inlineImageInfo != null) {
            imageObject = PdfImageObject(inlineImageInfo.imageDictionary, inlineImageInfo.samples, colorSpaceDictionary)
        }
    }
    ...
    val startPoint: Vector
        get() = Vector(0f, 0f, 1f).cross(gs.ctm)
     ...
     * @since 5.5.7
     */
    val currentFillColor: BaseColor
        get() = gs.fillColor
    ...
    companion object {
         ...
         * @since 5.0.1
         */
        fun createForXObject(gs: GraphicsState, ref: PdfIndirectReference, colorSpaceDictionary: PdfDictionary): ImageRenderInfo {
            return ImageRenderInfo(gs, ref, colorSpaceDictionary)
        }
         ...
         * @since 5.0.1
         */
        protected fun createForEmbeddedImage(gs: GraphicsState, inlineImageInfo: InlineImageInfo, colorSpaceDictionary: PdfDictionary): ImageRenderInfo {
            val renderInfo = ImageRenderInfo(gs, inlineImageInfo, colorSpaceDictionary)
            return renderInfo
        }
    }
}
