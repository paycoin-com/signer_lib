 ...
 * @author Paulo Soares
 */
class PdfStamper : PdfViewerPreferences, PdfEncryptionSettings {
    /**
     * The writer
     */
    protected var stamper: PdfStamperImp
    ...
    constructor(reader: PdfReader, os: OutputStream) {
        stamper = PdfStamperImp(reader, os, '\0', false)
    }
     ...
     */
    @Throws(DocumentException::class, IOException::class)
    constructor(reader: PdfReader, os: OutputStream, pdfVersion: Char) {
        stamper = PdfStamperImp(reader, os, pdfVersion, false)
    }
     ...
     * * document
     * *
     * @param append if true appends the document changes as a new revision. This is
     ...
     */
    @Throws(DocumentException::class, IOException::class)
    constructor(reader: PdfReader, os: OutputStream, pdfVersion: Char, append: Boolean) {
        stamper = PdfStamperImp(reader, os, pdfVersion, append)
    }
     ...
     * @param r the PdfReader from where the new page will be imported
     * *
    ...
    fun close() {
        ...
        if (!hasSignature) {
            ...
            throw DocumentException("Signature defined. Must be closed in PdfSignatureAppearance.")
        }
    }
     ...
     * *
     * @return a PdfContentByte to write under the page of
     * * the original document
     */
     ...
     * @param pageNum the page number where the extra content is written
     * *
     * @return a PdfContentByte to write over the page of
     * * the original document
     */
     ...
     * @param rotateContents true to set auto-rotation, false
     * * otherwise
     */
    var isRotateContents: Boolean
        get() = stamper.isRotateContents
        set(rotateContents) {
            stamper.isRotateContents = rotateContents
        }
    ...
    fun setEncryption(userPassword: ByteArray, ownerPassword: ByteArray, permissions: Int, strength128Bits: Boolean) {
        if (stamper.isAppend)
            throw DocumentException(MessageLocalization.getComposedMessage("append.mode.does.not.support.changing.the.encryption.status"))
        if (stamper.isContentWritten)
            throw DocumentException(MessageLocalization.getComposedMessage("content.was.already.written.to.the.output"))
        stamper.setEncryption(userPassword, ownerPassword, permissions, if (strength128Bits) PdfWriter.STANDARD_ENCRYPTION_128 else PdfWriter.STANDARD_ENCRYPTION_40)
    }
     ...
     * @param flat true to flatten the FreeText annotations, false
     ...
     */
    fun setAnnotationFlattening(flat: Boolean) {
        stamper.setFlatAnnotations(flat)
    }

    /**
     * Adds an annotation of form field in a specific page. This page number
     * can be overridden with [PdfAnnotation.setPlaceInPage].
     * @param annot the annotation
     * *
     * @param page the page
     */
    fun addAnnotation(annot: PdfAnnotation, page: Int) {
     ...
     * @since    2.1.4
     */
    fun addSignature(name: String, page: Int, llx: Float, lly: Float, urx: Float, ury: Float): PdfFormField {
        val acroForm = stamper.acroForm
        val signature = PdfFormField.createSignature(stamper)
     ...
     * @param js the JavaScript code
     */
    fun addJavaScript(name: String, js: String) {
     ...
     */
    fun makePackage(initialView: PdfName) {
        val collection = PdfCollection(0)
     ...
     * @param xmp
     * *
     * @see PdfWriter.setXmpMetadata
     */
    fun setXmpMetadata(xmp: ByteArray) {
        stamper.setXmpMetadata(xmp)
    }

    fun createXmpMetadata() {
        stamper.createXmpMetadata()
    }

    val xmpWriter: XmpWriter
        get() = stamper.xmpWriter
    ...
    fun setFullCompression() {
        if (stamper.isAppend)
            return
        stamper.isFullCompression = true
    ...
    val pdfLayers: Map<String, PdfLayer>
        get() = stamper.pdfLayers

    fun markUsed(obj: PdfObject) {
        stamper.markUsed(obj)
    }

    val ltvVerification: LtvVerification
        get() {
            if (verification == null)
                verification = LtvVerification(this)
            return verification
        }

    @Throws(IOException::class)
    fun addNamedDestination(name: String, page: Int, dest: PdfDestination): Boolean {
        val namedDestinations = stamper.namedDestinations

        if (reader.namedDestination.containsKey(name)) {
            return false
        }
        val d = PdfDestination(dest)
        d.addPage(reader.getPageOrigRef(page))
        namedDestinations.put(name, PdfArray(d))
        return true
    }

    @Throws(IOException::class)
    internal fun mergeVerification() {
        if (verification == null)
            return
        verification!!.merge()
    }
    ...
    companion object {
         ...
         * *
         * @param os the output stream or null to keep the document in the temporary file
         * *
         * @param pdfVersion the new pdf version or '\0' to keep the same version as the original
         * * document
         * *
         ...
         * *
         * @param append if true the signature and all the other content will be added as a
         ...
         */
        @Throws(DocumentException::class, IOException::class)
        @JvmOverloads fun createSignature(reader: PdfReader, os: OutputStream, pdfVersion: Char, tempFile: File? = null, append: Boolean = false): PdfStamper {
            var tempFile = tempFile
            val stp: PdfStamper
            if (tempFile == null) {
                val bout = ByteBuffer()
                stp = PdfStamper(reader, bout, pdfVersion, append)
                stp.signatureAppearance = PdfSignatureAppearance(stp.stamper)
                stp.signatureAppearance.sigout = bout
            } else {
                if (tempFile.isDirectory)
                    tempFile = File.createTempFile("pdf", ".pdf", tempFile)
                val fout = FileOutputStream(tempFile)
                stp = PdfStamper(reader, fout, pdfVersion, append)
                stp.signatureAppearance = PdfSignatureAppearance(stp.stamper)
                stp.signatureAppearance.tempFile = tempFile
            }
            stp.signatureAppearance.originalout = os
            stp.signatureAppearance.stamper = stp
            stp.hasSignature = true
            val catalog = reader.catalog
            val acroForm = PdfReader.getPdfObject(catalog.get(PdfName.ACROFORM), catalog) as PdfDictionary?
            if (acroForm != null) {
        ...
        fun createXmlSignature(reader: PdfReader, os: OutputStream): PdfStamper {
            val stp = PdfStamper(reader, os)
            stp.xmlSignatureAppearance = XmlSignatureAppearance(stp.stamper)
            //stp.sigApp.setSigout(bout);
            //stp.sigApp.setOriginalout(os);
            stp.xmlSignatureAppearance.stamper = stp
