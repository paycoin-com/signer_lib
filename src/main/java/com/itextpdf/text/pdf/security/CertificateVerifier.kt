 ...
 */
open class CertificateVerifier
/**
...
(
        ...
        protected var verifier: CertificateVerifier?) {

    /** Indicates if going online to verify a certificate is allowed.  */
    protected var onlineCheckingAllowed = true

    /**
     * Decide whether or not online checking is allowed.
     * @param onlineCheckingAllowed
     */
    fun setOnlineCheckingAllowed(onlineCheckingAllowed: Boolean) {
    ...
    open fun verify(signCert: X509Certificate, issuerCert: X509Certificate?, signDate: Date?): List<VerificationOK> {
        // Check if the certificate is valid on the signDate
        if (signDate != null)
            signCert.checkValidity(signDate)
        // Check if the signature is valid
        if (issuerCert != null) {
            signCert.verify(issuerCert.publicKey)
        } else {
