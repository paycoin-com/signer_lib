...
@JvmOverloads constructor(initialCapacity: Int = 150,
     ...
     */
    protected constructor(var hash: Int, /*
...
*/
            // extra methods for inner class Entry by Paulo
                          var key: Int, var value: Int, //ArrayList<Integer> values = new ArrayList<Integer>();
                          var next: Entry?)//values.add(value);
    {
    ...
    internal class IntHashtableIterator(var table: Array<Entry>) : Iterator<Entry> {
        var index: Int = 0
    ...
    val entryIterator: Iterator<Entry>
        get() = IntHashtableIterator(table)

    fun toOrderedKeys(): IntArray {
        val res = keys
        Arrays.sort(res)
        return res
    }
    ...
    public override fun clone(): Any {
        try {
            val t = super.clone() as IntHashtable
