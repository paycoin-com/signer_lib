...
class BidiLine {
    ...
    protected var runDirection: Int = 0
    protected var pieceSize = 256
    protected var text = CharArray(pieceSize)
    protected var detailChunks = arrayOfNulls<PdfChunk>(pieceSize)
    protected var totalTextLength = 0

    protected var orderLevels = ByteArray(pieceSize)
    protected var indexChars = IntArray(pieceSize)

    protected var chunks = ArrayList<PdfChunk>()
    protected var indexChunk = 0
    protected var indexChunkChar = 0
    protected var currentChar = 0

    protected var storedRunDirection: Int = 0
    protected var storedText = CharArray(0)
    protected var storedDetailChunks = arrayOfNulls<PdfChunk>(0)
    protected var storedTotalTextLength = 0

    protected var storedOrderLevels = ByteArray(0)
    protected var storedIndexChars = IntArray(0)

    protected var storedIndexChunk = 0
    protected var storedIndexChunkChar = 0
    protected var storedCurrentChar = 0
    ...
    protected var shortStore: Boolean = false
    protected var arabicOptions: Int = 0

    /** Creates new BidiLine  */
    constructor() {
    }

    constructor(org: BidiLine) {
    ...
    val isEmpty: Boolean
        get() = currentChar >= totalTextLength && indexChunk >= chunks.size
    ...
    fun getParagraph(runDirection: Int): Boolean {
        ...
        var uniC: Char
        var bf: BaseFont
        while (indexChunk < chunks.size) {
            val ck = chunks[indexChunk]
            bf = ck.font().font
            val s = ck.toString()
            val len = s.length
            while (indexChunkChar < len) {
                c = s[indexChunkChar]
                uniC = bf.getUnicodeEquivalent(c.toInt()).toChar()
                if (uniC == '\r' || uniC == '\n') {
        ...
        if (runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL) {
            if (orderLevels.size < totalTextLength) {
                orderLevels = ByteArray(pieceSize)
                indexChars = IntArray(pieceSize)
            }
            ArabicLigaturizer.processNumbers(text, 0, totalTextLength, arabicOptions)
            val order = BidiOrder(text, 0, totalTextLength, (if (runDirection == PdfWriter.RUN_DIRECTION_RTL) 1 else 0).toByte())
            val od = order.levels
            for (k in 0..totalTextLength - 1) {
                orderLevels[k] = od[k]
                indexChars[k] = k
            }
            doArabicShapping()
    ...
    fun addChunk(chunk: PdfChunk) {
        chunks.add(chunk)
    }

    fun addChunks(chunks: ArrayList<PdfChunk>) {
        this.chunks.addAll(chunks)
    }

    fun addPiece(c: Char, chunk: PdfChunk) {
        if (totalTextLength >= pieceSize) {
            val tempText = text
            val tempDetailChunks = detailChunks
            pieceSize *= 2
            text = CharArray(pieceSize)
            detailChunks = arrayOfNulls<PdfChunk>(pieceSize)
    ...
    fun save() {
        ...
        if (!shortStore) {
            // long save
            if (storedText.size < totalTextLength) {
                storedText = CharArray(totalTextLength)
                storedDetailChunks = arrayOfNulls<PdfChunk>(totalTextLength)
            }
            System.arraycopy(text, 0, storedText, 0, totalTextLength)
            System.arraycopy(detailChunks, 0, storedDetailChunks, 0, totalTextLength)
        }
        if (runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL) {
            if (storedOrderLevels.size < totalTextLength) {
                storedOrderLevels = ByteArray(totalTextLength)
                storedIndexChars = IntArray(totalTextLength)
            }
    ...
    fun mirrorGlyphs() {
        for (k in 0..totalTextLength - 1) {
            if (orderLevels[k] and 1 == 1) {
                val mirror = mirrorChars.get(text[k].toInt())
                if (mirror != 0)
                    text[k] = mirror.toChar()
            }
        }
    }

    fun doArabicShapping() {
        var src = 0
        var dest = 0
        while (true) {
            while (src < totalTextLength) {
                val c = text[src]
                if (c.toInt() >= 0x0600 && c.toInt() <= 0x06ff)
                    break
                if (src != dest) {
                    text[dest] = text[src]
            ...
            val arabicWordSize = src - startArabicIdx
            val size = ArabicLigaturizer.arabic_shape(text, startArabicIdx, arabicWordSize, text, dest, arabicWordSize, arabicOptions)
    ...
    fun processLine(leftX: Float, width: Float, alignment: Int, runDirection: Int, arabicOptions: Int, minY: Float, yLine: Float, descender: Float): PdfLine? {
        var width = width
        isWordSplit = false
        ...
        if (currentChar >= totalTextLength) {
            val hasText = getParagraph(runDirection)
            if (!hasText)
                return null
            if (totalTextLength == 0) {
                val ar = ArrayList<PdfChunk>()
                val ck = PdfChunk("", detailChunks[0])
                ar.add(ck)
                return PdfLine(0f, 0f, width, alignment, true, ar, isRTL)
            }
        }
        ...
        val oldCurrentChar = currentChar
        var uniC = 0
        var ck: PdfChunk? = null
        var charWidth = 0f
        var lastValidChunk: PdfChunk? = null
        var tabStop: TabStop? = null
        var tabStopAnchorPosition = java.lang.Float.NaN
        var tabPosition = java.lang.Float.NaN
        var surrogate = false
        while (currentChar < totalTextLength) {
            ck = detailChunks[currentChar]
            if (ck!!.isImage && minY < yLine) {
                val img = ck.getImage()
                if (img.isScaleToFitHeight && yLine + 2 * descender - img.scaledHeight - ck.imageOffsetY - img.getSpacingBefore() < minY) {
                    val scalePercent = (yLine + 2 * descender - ck.imageOffsetY - img.getSpacingBefore() - minY) / img.scaledHeight
            ...
            if (surrogate)
                ...
                uniC = ck.getUnicodeEquivalent(text[currentChar].toInt())
            ...
            if (surrogate)
                ...
                if (ck.isImage) {
                    ...
                    charWidth = ck.getCharWidth(text[currentChar].toInt())
                }
            }
            if (width - charWidth < 0) {
                ...
                if (lastValidChunk == null && ck.isImage) {
                    ...
                    if (img.isScaleToFitLineWhenOverflow) {
                        ...
                        val scalePercent = width / img.width
                        ck.setImageScalePercentage(scalePercent)
                        charWidth = width
                    }
                }
            ...
            if (ck.isTab) {
                if (ck.isAttribute(Chunk.TABSETTINGS)) {
                    ...
                    val tab = ck.getAttribute(Chunk.TAB) as Array<Any>
                    ...
                    if (newLine && tabStopPosition < originalWidth - width) {
                        return PdfLine(0f, originalWidth, width, alignment, true, createArrayOfPdfChunks(oldCurrentChar, currentChar - 1), isRTL)
                    }
                    detailChunks[currentChar].adjustLeft(leftX)
                    width = originalWidth - tabStopPosition
                }
            } else if (ck.isSeparator) {
                val sep = ck.getAttribute(Chunk.SEPARATOR) as Array<Any>
                val di = sep[0] as DrawInterface
                val vertical = sep[1] as Boolean
                if (vertical && di is LineSeparator) {
                    val separatorWidth = originalWidth * di.percentage / 100f
                ...
                val splitChar = ck.isExtSplitCharacter(oldCurrentChar, currentChar, totalTextLength, text, detailChunks)
                if (splitChar && Character.isWhitespace(uniC.toChar()))
                    lastSplit = currentChar
                if (width - charWidth < 0)
                    break
                if (tabStop != null && tabStop.alignment == TabStop.Alignment.ANCHOR && java.lang.Float.isNaN(tabStopAnchorPosition) && tabStop.anchorChar == uniC.toChar()) {
        ...
        if (lastValidChunk == null) {
            ...
            if (surrogate)
                ++currentChar
            return PdfLine(0f, originalWidth, 0f, alignment, false, createArrayOfPdfChunks(currentChar - 1, currentChar - 1), isRTL)
        }
        ...
        if (currentChar >= totalTextLength) {
            ...
            return PdfLine(0f, originalWidth, width, alignment, true, createArrayOfPdfChunks(oldCurrentChar, totalTextLength - 1), isRTL)
        }
        var newCurrentChar = trimRightEx(oldCurrentChar, currentChar - 1)
        if (newCurrentChar < oldCurrentChar) {
            // only WS
            return PdfLine(0f, originalWidth, width, alignment, false, createArrayOfPdfChunks(oldCurrentChar, currentChar - 1), isRTL)
        }
        if (newCurrentChar == currentChar - 1) {
            // middle of word
            val he = lastValidChunk.getAttribute(Chunk.HYPHENATION) as HyphenationEvent
            if (he != null) {
                val word = getWord(oldCurrentChar, newCurrentChar)
                if (word != null) {
                    val testWidth = width + getWidth(word[0], currentChar - 1)
                    val pre = he.getHyphenatedWordPre(String(text, word[0], word[1] - word[0]), lastValidChunk.font().font, lastValidChunk.font().size(), testWidth)
                    val post = he.hyphenatedWordPost
                    if (pre.length > 0) {
                        val extra = PdfChunk(pre, lastValidChunk)
                        currentChar = word[1] - post.length
                        return PdfLine(0f, originalWidth, testWidth - lastValidChunk.width(pre), alignment, false, createArrayOfPdfChunks(oldCurrentChar, word[0] - 1, extra), isRTL)
                    }
                }
            }
        }
        ...
        if (lastSplit == -1 || lastSplit >= newCurrentChar) {
            ...
            return PdfLine(0f, originalWidth, width + getWidth(newCurrentChar + 1, currentChar - 1, originalWidth), alignment, false, createArrayOfPdfChunks(oldCurrentChar, newCurrentChar), isRTL)
        }
        // standard split
        ...
        return PdfLine(0f, originalWidth, originalWidth - getWidth(oldCurrentChar, newCurrentChar, originalWidth), alignment, false, createArrayOfPdfChunks(oldCurrentChar, newCurrentChar), isRTL)
    }
     ...
     */
    @JvmOverloads fun getWidth(startIdx: Int, lastIdx: Int, originalWidth: Float = 0f): Float {
        ...
        while (startIdx <= lastIdx) {
            val surrogate = Utilities.isSurrogatePair(text, startIdx)
            if (detailChunks[startIdx].isTab() //Keep deprecated tab logic for backward compatibility...
                    && detailChunks[startIdx].isAttribute(Chunk.TABSETTINGS)) {
                if (tabStop != null) {
                    val tabStopPosition = tabStop.getPosition(tabPosition, width, tabStopAnchorPosition)
                ...
                if (PdfChunk.noPrint(ck!!.getUnicodeEquivalent(c.toInt()))) {
                    ++startIdx
                    continue
                }
                if (tabStop != null && tabStop.alignment != TabStop.Alignment.ANCHOR && java.lang.Float.isNaN(tabStopAnchorPosition) && tabStop.anchorChar == ck.getUnicodeEquivalent(c.toInt()).toChar()) {
                    tabStopAnchorPosition = width
                }
                width += detailChunks[startIdx].getCharWidth(c.toInt())
            }
            ++startIdx
        }
        if (tabStop != null) {
    ...
    @JvmOverloads fun createArrayOfPdfChunks(startIdx: Int, endIdx: Int, extraPdfChunk: PdfChunk? = null): ArrayList<PdfChunk> {
        var startIdx = startIdx
        val bidi = runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL
        if (bidi)
        ...
        while (startIdx <= endIdx) {
            idx = if (bidi) indexChars[startIdx] else startIdx
            c = text[idx]
            ck = detailChunks[idx]
            if (PdfChunk.noPrint(ck!!.getUnicodeEquivalent(c.toInt()))) {
                ++startIdx
            ...
            if (ck.isImage || ck.isSeparator || ck.isTab) {
                if (buf.length > 0) {
                    ar.add(PdfChunk(buf.toString(), refCk))
                    buf = StringBuffer()
                }
                ar.add(ck)
            } else if (ck === refCk) {
    ...
    fun getWord(startIdx: Int, idx: Int): IntArray? {
        var last = idx
        var first = idx
        // forward
        while (last < totalTextLength) {
    ...
    fun trimRight(startIdx: Int, endIdx: Int): Int {
        var idx = endIdx
        var c: Char
        while (idx >= startIdx) {
            c = detailChunks[idx].getUnicodeEquivalent(text[idx].toInt()).toChar()
            if (!isWS(c))
                break
            --idx
        }
    ...
    fun trimLeft(startIdx: Int, endIdx: Int): Int {
        var idx = startIdx
        var c: Char
        while (idx <= endIdx) {
            c = detailChunks[idx].getUnicodeEquivalent(text[idx].toInt()).toChar()
    ...
    fun trimRightEx(startIdx: Int, endIdx: Int): Int {
        ...
        while (idx >= startIdx) {
            c = detailChunks[idx].getUnicodeEquivalent(text[idx].toInt()).toChar()
            if (!isWS(c) && !PdfChunk.noPrint(c.toInt())) {
                if (detailChunks[idx].isTab() //Keep deprecated tab logic for backward compatibility...
                        && detailChunks[idx].isAttribute(Chunk.TABSETTINGS)) {
                    val tab = detailChunks[idx].getAttribute(Chunk.TAB) as Array<Any>
    ...
    fun reorder(start: Int, end: Int) {
        ...
        for (k in start + 1..end) {
            ...
            if (b > maxLevel)
                ...
                minLevel = b
            onlyOddLevels = onlyOddLevels and b
            onlyEvenLevels = onlyEvenLevels or b
        }
        if (onlyEvenLevels and 1 == 0)
        // nothing to do
            return
        if (onlyOddLevels and 1 == 1) {
            // single inversion
            flip(start, end + 1)
            return
        }
    ...
    companion object {
        ...
        protected val mirrorChars = IntHashtable()
         ...
         */
        fun processLTR(s: String, runDirection: Int, arabicOptions: Int): String {
            val bidi = BidiLine()
            bidi.addChunk(PdfChunk(Chunk(s), null))
            bidi.arabicOptions = arabicOptions
            bidi.getParagraph(runDirection)
            val arr = bidi.createArrayOfPdfChunks(0, bidi.totalTextLength - 1)
