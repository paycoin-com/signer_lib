...
object Base64 {
    ...
    private fun getAlphabet(options: Int): ByteArray {
        if (options and URL_SAFE == URL_SAFE)
     ...
     * @since 2.0
     */
    @JvmOverloads fun encodeObject(serializableObject: java.io.Serializable, options: Int = NO_OPTIONS): String? {
        // Streams
     ...
     * @since 2.0
     */
    @JvmOverloads fun encodeBytes(source: ByteArray, off: Int = 0, len: Int = source.size, options: Int = NO_OPTIONS): String? {
        // Isolate options
        val dontBreakLines = options and DONT_BREAK_LINES
        val gzip = options and GZIP
     ...
     * @since 1.3
     */
    private fun decode4to3(source: ByteArray, srcOffset: Int, destination: ByteArray, destOffset: Int, options: Int): Int {
        ...
        if (source[srcOffset + 2] == EQUALS_SIGN) {
            ...
            try {
                ...
                println("" + source[srcOffset] + ": " + DECODABET[source[srcOffset]])
                println("" + source[srcOffset + 1] + ": " + DECODABET[source[srcOffset + 1]])
                println("" + source[srcOffset + 2] + ": " + DECODABET[source[srcOffset + 2]])
                println("" + source[srcOffset + 3] + ": " + DECODABET[source[srcOffset + 3]])
                return -1
            }
            // end catch
        }// Example: DkLE
     ...
     * @since 1.4
     */
    @JvmOverloads fun decode(s: String, options: Int = NO_OPTIONS): ByteArray {
        var bytes: ByteArray?
        try {
        ...
        if (bytes != null && bytes.size >= 4) {
            ...
            val head = bytes[0] and 0xff or (bytes[1] shl 8 and 0xff00)
    ...
    fun encodeFromFile(filename: String): String {
        ...
        try {
            // Set up some useful variables
            val file = java.io.File(filename)
            val buffer = ByteArray(Math.max((file.length() * 1.4).toInt(), 40)) // Need max() for math on small files (v2.2.1)
            var length = 0
            var numBytes = 0
    ...
    @JvmOverloads constructor(`in`: java.io.InputStream, private val options: Int = DECODE        // Record options used to create the stream.
        ...
        private val alphabet: ByteArray        // Local copies to avoid extra method calls
        ...
        override fun read(): Int {
            ...
            if (position < 0) {
                if (encode) {
                    ...
                    if (i == 4) {
                        ...
                        throw java.io.IOException(MessageLocalization.getComposedMessage("improperly.padded.base64.input"))
                    }   // end
            ...
            if (position >= 0) {
                ...
                throw java.io.IOException(MessageLocalization.getComposedMessage("error.in.base64.code.reading.stream"))
            }// Else error
            // end else
        }   // end read
    ...
    @JvmOverloads constructor(out: java.io.OutputStream, private val options: Int = ENCODE // Record for later
        ...
        override fun write(theByte: Int) {
            ...
            if (encode) {
                ...
                if (decodabet[theByte and 0x7f] > WHITE_SPACE_ENC) {
                    ...
                    throw java.io.IOException(MessageLocalization.getComposedMessage("invalid.character.in.base64.data"))
        ...
        /**
         * Calls [.write] repeatedly until len
         * bytes are written.
        ...
        fun flushBase64() {
            if (position > 0) {
                if (encode) {
                    ...
                    throw java.io.IOException(MessageLocalization.getComposedMessage("base64.input.not.properly.padded"))
                }   // end else: decoding
