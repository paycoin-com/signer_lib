...
(key: ByteArray, off: Int, len: Int, revision: Int) {
    ...
    private var ivptr: Int = 0

    init {
        aes = revision == AES_128 || revision == AES_256
        if (aes) {
            this.key = ByteArray(len)
    ...
    fun update(b: ByteArray, off: Int, len: Int): ByteArray? {
        var off = off
        var len = len
        if (aes) {
            if (initiated)
                ...
                val left = Math.min(iv.size - ivptr, len)
                System.arraycopy(b, off, iv, ivptr, left)
                off += left
                len -= left
                ivptr += left
                if (ivptr == iv.size) {
                    cipher = AESCipher(false, key, iv)
                    initiated = true
                    if (len > 0)
                        return cipher.update(b, off, len)
                }
