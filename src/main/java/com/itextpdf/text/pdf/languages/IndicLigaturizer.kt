 ...
 */
abstract class IndicLigaturizer : LanguageProcessor {

    /**
     * The table mapping specific character indexes to the characters in a
     * specific language.
     */
    protected var langTable: CharArray
    ...
    override fun process(s: String?): String {
        ...
        for (i in 0..s.length - 1) {
            ...
            if (IsVyanjana(letter) || IsSwaraLetter(letter)) {
                ...
                val prevCharIndex = res.length - 1
                ...
                if (prevCharIndex >= 0) {
                    ...
                    res.append(letter)
                    val prevPrevCharIndex = res.length - 2
     ...
     */
    protected fun IsSwaraLetter(ch: Char): Boolean {
     ...
     */
    protected fun IsSwaraMatra(ch: Char): Boolean {
     ...
     */
    protected fun IsVyanjana(ch: Char): Boolean {
    ...
    companion object {
         ...
         */
        private fun swap(s: StringBuilder, i: Int, j: Int) {
            val temp = s[i]
            s.setCharAt(i, s[j])
            s.setCharAt(j, temp)
        }
