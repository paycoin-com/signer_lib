 ...
 */
(writer: PdfWriter) : PdfDictionary(PdfName.OCMD), PdfOCG {
    ...
    override var ref: PdfIndirectReference
        internal set(value: PdfIndirectReference) {
            super.ref = value
        }
    internal var members = PdfArray()
    internal var layers = HashSet<PdfLayer>()
     ...
     */
    fun getLayers(): Collection<PdfLayer> {
        return layers
    }
     ...
     */
    fun setVisibilityPolicy(type: PdfName) {
        put(PdfName.P, type)
    }
     ...
     * @since 5.0.2
     */
    fun setVisibilityExpression(ve: PdfVisibilityExpression) {
        put(PdfName.VE, ve)
    }
