 ...
 * @since    2.1.3
 */
@JvmOverloads internal constructor(painter: PdfPatternPainter, compressionLevel: Int = PdfStream.DEFAULT_COMPRESSION) : PdfStream() {

    init {
        val one = PdfNumber(1)
        val matrix = painter.matrix
        if (matrix != null) {
            put(PdfName.MATRIX, matrix)
        }
        put(PdfName.TYPE, PdfName.PATTERN)
        put(PdfName.BBOX, PdfRectangle(painter.boundingBox))
        put(PdfName.RESOURCES, painter.resources)
        put(PdfName.TILINGTYPE, one)
        put(PdfName.PATTERNTYPE, one)
        if (painter.isStencil)
            put(PdfName.PAINTTYPE, PdfNumber(2))
        else
            put(PdfName.PAINTTYPE, one)
        put(PdfName.XSTEP, PdfNumber(painter.xStep))
        put(PdfName.YSTEP, PdfNumber(painter.yStep))
