    ...
    extends TestCase
{
    ...
    {
        Companion.assertEquals(arr1.length, arr2.length);
        for (int i = 0; i < arr1.length; i++)
        {
            Companion.assertEquals((arr1[i] + m) % m, (arr2[i] + m) % m);
        }
    }
    ...
    public void testToIntegerPolynomial()
    {
        ...
        Companion.assertTrue(Arrays.areEqual(coeffs, p.toIntegerPolynomial().coeffs));
    }
}