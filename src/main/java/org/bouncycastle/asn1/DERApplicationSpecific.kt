...
class DERApplicationSpecific : ASN1ApplicationSpecific {
    ...
    constructor(
            tag: Int,
            octets: ByteArray) : this(false, tag, octets) {
    }
    ...
    private fun getEncoding(explicit: Boolean, `object`: ASN1Encodable): ByteArray {
        val data = `object`.toASN1Primitive().getEncoded(ASN1Encoding.DER)
    ...
    private fun getEncodedVector(vec: ASN1EncodableVector): ByteArray {
        ...
        for (i in 0..vec.size() - 1) {
            try {
                bOut.write((vec.get(i) as ASN1Object).getEncoded(ASN1Encoding.DER))
            } catch (e: IOException) {
