...
class ParseTest : TestCase() {
    ...
    internal var longTagged = Hex.decode("9f1f023330")
    ...
    @Throws(IOException::class)
    fun testLongTag() {
        val aIn = ASN1StreamParser(longTagged)

        val tagged = aIn.readObject() as ASN1TaggedObjectParser

        TestCase.Companion.assertEquals(31, tagged.tagNo)
    }

    @Throws(IOException::class)
    private fun parseEnveloped(data: ByteArray) {
        val aIn = ASN1StreamParser(data)

        val cP = ContentInfoParser(aIn.readObject() as ASN1SequenceParser)

        val eP = EnvelopedDataParser(cP.getContent(BERTags.SEQUENCE) as ASN1SequenceParser?)

        eP.recipientInfos.toASN1Primitive() // Must drain the parser!

        val ecP = eP.encryptedContentInfo

        val content = ecP.getEncryptedContent(BERTags.OCTET_STRING) as ASN1OctetStringParser

        Streams.drain(content.octetStream)
    }

    companion object {
        private val classCastTest = Base64.decode(
        ...
        private val derExpTest = Base64.decode(
