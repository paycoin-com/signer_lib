...
class OCSPTest : Test {
    private val unsignedReq = Base64.decode(
            "MEIwQDA+MDwwOjAJBgUrDgMCGgUABBRDb9GODnq7lRhSkEqw4XX24huERwQUkY4j" + "a6eKuDlkVP9hRgkEvIWqHPECAQE=")

    private val signedReq = Base64.decode(
    ...
    private val response = Base64.decode(
    ...
    private fun unsignedRequest(): TestResult {
        try {
            val aIn = ASN1InputStream(ByteArrayInputStream(unsignedReq))
            val req = OCSPRequest.getInstance(aIn.readObject())

            if (!isSameAs(req.getEncoded(), unsignedReq)) {
                return SimpleTestResult(false, name + ": OCSP unsigned request failed to re-encode")
            }

            return SimpleTestResult(true, name + ": Okay")
        } catch (e: Exception) {
            return SimpleTestResult(false, name + ": failed unsigned exception - " + e.toString(), e)
        }

    }

    private fun signedRequest(): TestResult {
        try {
            val aIn = ASN1InputStream(ByteArrayInputStream(signedReq))
            val req = OCSPRequest.getInstance(aIn.readObject())

            if (!isSameAs(req.getEncoded(), signedReq)) {
                return SimpleTestResult(false, name + ": OCSP signed request failed to re-encode")
            }

            return SimpleTestResult(true, name + ": Okay")
        } catch (e: Exception) {
            return SimpleTestResult(false, name + ": failed signed exception - " + e.toString(), e)
        }

    }

    private fun response(): TestResult {
        try {
            var aIn = ASN1InputStream(ByteArrayInputStream(response))
            var resp = OCSPResponse.getInstance(aIn.readObject())
            val rBytes = ResponseBytes.getInstance(resp.responseBytes)

            aIn = ASN1InputStream(ByteArrayInputStream(rBytes.response.octets))

            val bResp = BasicOCSPResponse.getInstance(aIn.readObject())

            resp = OCSPResponse(resp.responseStatus, ResponseBytes(rBytes.responseType, DEROctetString(bResp.getEncoded())))

            if (!isSameAs(resp.encoded, response)) {
                return SimpleTestResult(false, name + ": OCSP response failed to re-encode")
            }

            return SimpleTestResult(true, name + ": Okay")
        } catch (e: Exception) {
            return SimpleTestResult(false, name + ": failed response exception - " + e.toString(), e)
        }

    }

    override fun perform(): TestResult {
        var res = unsignedRequest()
