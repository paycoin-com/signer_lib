...
class PKCS12Test : SimpleTest() {
    internal var pkcs12 = Base64.decode(
    ...
    override fun performTest() {
        var aIn = ASN1InputStream(ByteArrayInputStream(pkcs12))
        val obj = aIn.readObject() as ASN1Sequence
        var bag = Pfx.getInstance(obj)
        var info: ContentInfo = bag.authSafe
        var mData: MacData = bag.macData
        val dInfo = mData.mac
        val algId = dInfo.algorithmId
        val salt = mData.salt
        val itCount = mData.iterationCount.intValue()

        aIn = ASN1InputStream(ByteArrayInputStream((info.content as ASN1OctetString).octets))

        var authSafe = AuthenticatedSafe.getInstance(aIn.readObject())
        val c = authSafe.contentInfo
        ...
        aIn = ASN1InputStream(ByteArrayInputStream((c[0].content as ASN1OctetString).octets))
        val seq = aIn.readObject() as ASN1Sequence

        var b = SafeBag.getInstance(seq.getObjectAt(0))
        if (!b.bagId!!.equals(PKCSObjectIdentifiers.pkcs8ShroudedKeyBag)) {
            fail("failed comparison shroudedKeyBag test")
        }

        var encInfo = EncryptedPrivateKeyInfo.getInstance(b.bagValue)

        encInfo = EncryptedPrivateKeyInfo(encInfo.encryptionAlgorithm, encInfo.encryptedData)

        b = SafeBag(PKCSObjectIdentifiers.pkcs8ShroudedKeyBag, encInfo.toASN1Primitive(), b.bagAttributes)

        var abOut = ByteArrayOutputStream()
        var berOut = ASN1OutputStream(abOut)

        berOut.writeObject(DLSequence(b))

        c[0] = ContentInfo(PKCSObjectIdentifiers.data, BEROctetString(abOut.toByteArray()))
        ...
        val eData = EncryptedData.getInstance(c[1].content)

        c[1] = ContentInfo(PKCSObjectIdentifiers.encryptedData, eData)

        //
        // create an octet stream represent the BER encoding of authSafe
        //
        authSafe = AuthenticatedSafe(c)

        abOut = ByteArrayOutputStream()
        berOut = ASN1OutputStream(abOut)

        berOut.writeObject(authSafe)

        info = ContentInfo(PKCSObjectIdentifiers.data, BEROctetString(abOut.toByteArray()))

        mData = MacData(DigestInfo(algId, dInfo.digest), salt, itCount)
