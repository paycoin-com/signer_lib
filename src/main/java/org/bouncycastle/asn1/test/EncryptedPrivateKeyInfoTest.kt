 ...
 */
class EncryptedPrivateKeyInfoTest : SimpleTest() {
    ...
    private fun test(
            id: Int,
            sample: ByteArray) {
        var bIn = ByteArrayInputStream(sample)
        var aIn = ASN1InputStream(bIn)
        var info: EncryptedPrivateKeyInfo? = null
        ...
        if (bytes.size != sample.size) {
            try {
                ...
                fail("test " + id + " length mismatch - expected " + sample.size + Strings.lineSeparator() + ASN1Dump.dumpAsString(info) + " got " + bytes.size + Strings.lineSeparator() + ASN1Dump.dumpAsString(obj))
            } catch (e: Exception) {
    ...
    companion object {
        internal var sample1 = Base64.decode(
        ...
        internal var sample2 = Base64.decode(
        ...
        internal var sample3 = Base64.decode(
        ...
        @JvmStatic fun main(
                args: Array<String>) {
            SimpleTest.runTest(EncryptedPrivateKeyInfoTest())
        }
    }
}
