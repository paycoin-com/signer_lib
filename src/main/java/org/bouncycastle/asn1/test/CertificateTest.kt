...
class CertificateTest : SimpleTest() {
    //
    // server.crt
    //
    ...
    // ca.crt
    //
    internal var cert2 = Base64.decode(
    ...
    // testx509.pem
    //
    internal var cert3 = Base64.decode(
    ...
    // v3-cert1.pem
    //
    internal var cert4 = Base64.decode(
    ...
    // v3-cert2.pem
    //
    internal var cert5 = Base64.decode(
    ...
    internal var cert6 = Base64.decode(
    ...
    internal var cert7 = Base64.decode(
    ...
    internal var cert8 = Base64.decode(
    ...
    fun checkCertificate(
            id: Int,
            cert: ByteArray) {
        val bIn = ByteArrayInputStream(cert)
        val aIn = ASN1InputStream(bIn)

        val seq = aIn.readObject() as ASN1Sequence
        //        String dump = ASN1Dump.dumpAsString(seq);

        val obj = Certificate.getInstance(seq)
        val tbsCert = obj.tbsCertificate

        if (tbsCert.subject.toString() != subjects[id - 1]) {
            fail("failed subject test for certificate id " + id + " got " + tbsCert.subject.toString())
        }

        if (tbsCert.versionNumber == 3) {
            val ext = tbsCert.extensions
            if (ext != null) {
                val en = ext.oids()
                while (en.hasMoreElements()) {
                    val oid = en.nextElement() as ASN1ObjectIdentifier
                    val extVal = ext.getExtension(oid)

                    val oct = extVal.extnValue
                    val extIn = ASN1InputStream(ByteArrayInputStream(oct.octets))

                    if (oid == Extension.subjectKeyIdentifier) {
                        val si = SubjectKeyIdentifier.getInstance(extIn.readObject())

                        if (si != SubjectKeyIdentifier.fromExtensions(ext)) {
                            fail("SubjectKeyIdentifier not matched")
                        }
                    } else if (oid == Extension.keyUsage) {
                        val ku = KeyUsage.getInstance(extIn.readObject())

                        if (ku != KeyUsage.fromExtensions(ext)) {
                            fail("KeyUsage not matched")
                        }
                    } else if (oid == Extension.extendedKeyUsage) {
                        val ku = ExtendedKeyUsage.getInstance(extIn.readObject())

                        val sq = ku.toASN1Primitive() as ASN1Sequence
                        for (i in 0..sq.size() - 1) {
                            val p = ASN1ObjectIdentifier.getInstance(KeyPurposeId.getInstance(sq.getObjectAt(i)))
                        }

                        if (ku != ExtendedKeyUsage.fromExtensions(ext)) {
                            fail("ExtendedKeyUsage not matched")
                        }
                    } else if (oid == Extension.subjectAlternativeName) {
                        val gn = GeneralNames.getInstance(extIn.readObject())

                        val sq = gn.toASN1Primitive() as ASN1Sequence
                        for (i in 0..sq.size() - 1) {
                            val n = GeneralName.getInstance(sq.getObjectAt(i))
                        }
                    } else if (oid == Extension.issuerAlternativeName) {
                        val gn = GeneralNames.getInstance(extIn.readObject())
                    ...
                    } else if (oid == Extension.cRLDistributionPoints) {
                        val p = CRLDistPoint.getInstance(extIn.readObject())

                        val points = p.distributionPoints
                        for (i in points.indices) {
                            // do nothing
                        }
                    } else if (oid == Extension.certificatePolicies) {
                        val cp = extIn.readObject() as ASN1Sequence

                        for (i in 0..cp.size() - 1) {
                            PolicyInformation.getInstance(cp.getObjectAt(i))
                        }
                    } else if (oid == Extension.authorityKeyIdentifier) {
                        val auth = AuthorityKeyIdentifier.getInstance(extIn.readObject())

                        if (auth != AuthorityKeyIdentifier.fromExtensions(ext)) {
                            fail("AuthorityKeyIdentifier not matched")
                        }
                    } else if (oid == Extension.basicConstraints) {
                        val bc = BasicConstraints.getInstance(extIn.readObject())

                        if (bc != BasicConstraints.fromExtensions(ext)) {
    ...
    @Throws(Exception::class)
    fun checkAttributeCertificate(
            id: Int,
            cert: ByteArray) {
        val bIn: ByteArrayInputStream
        val aIn: ASN1InputStream

        bIn = ByteArrayInputStream(cert)
        aIn = ASN1InputStream(bIn)

        val seq = aIn.readObject() as ASN1Sequence
        //        String dump = ASN1Dump.dumpAsString(seq);

        val obj = AttributeCertificate.getInstance(seq)
        val acInfo = obj.acinfo

        // Version
        if (acInfo.version != ASN1Integer(1) && acInfo.version != ASN1Integer(2)) {
            fail(
        ...
        // Serial
        val serial = acInfo.serialNumber

        // Validity
        val validity = acInfo.attrCertValidityPeriod
        if (validity == null) {
            fail("failed AC AttCertValidityPeriod test for id " + id)
        }

        // Attributes
        val attribSeq = acInfo.attributes
        val att = arrayOfNulls<Attribute>(attribSeq.size())
        for (i in 0..attribSeq.size() - 1) {
    ...
    @Throws(Exception::class)
    fun checkV1AttributeCertificate(
            id: Int,
            cert: ByteArray) {
        val bIn: ByteArrayInputStream
        val aIn: ASN1InputStream
        ...
        // Version
        if (acInfo.version != ASN1Integer(0)) {
            fail(
                    "failed AC Version test for id " + id)
        }
    ...
    private fun checkDudCertificate() {
        val cert = Certificate.getInstance(dudCert)

        if ("" != cert.issuer.toString()) {
    ...
    companion object {
        ...
        private val attrCertv1 = Base64.decode(
        ...
        private val dudCert = Base64.decode(
        ...
        @JvmStatic fun main(
                args: Array<String>) {
            SimpleTest.runTest(CertificateTest())
        }
    }
}
