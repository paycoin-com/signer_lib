...
class ParsingTest : SimpleTest() {
    ...
    private fun parserTest() {
        for (i in streams.indices) {
            val aIn = ASN1StreamParser(Base64.decode(streams[i]))
    ...
    private fun inputStreamTest() {
        for (i in streams.indices) {
            val aIn = ASN1InputStream(Base64.decode(streams[i]))
    ...
    companion object {
        ...
        @JvmStatic fun main(
                args: Array<String>) {
            SimpleTest.runTest(ParsingTest())
        }
    }
}
